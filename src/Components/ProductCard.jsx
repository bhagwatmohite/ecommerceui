...
const ProductCard = ({ product }) => {
  ...
  const getFirstImageFromFirstSize = () => {
    if (product.sizes.length > 0) {
      const firstSize = product.sizes[0];
      if (firstSize.colors.length > 0) {
        const firstColor = firstSize.colors[0];
        if (firstColor.images.length > 0) {
          return firstColor.images[0]; // Return the first image from the first color
        }
      }
    }
  ...
  return (
    ...
    >
      <div style={imageContainerStyles} onClick={handleCardClick}>
        <Card.Img
          variant="top"
          src={`http://localhost:8080/ColorImages/${getFirstImageFromFirstSize() || 'default_image.jpg'}`} // Fallback to a default image if no images are found
          alt={product.name}
          style={imageStyles}
        />
      </div>
