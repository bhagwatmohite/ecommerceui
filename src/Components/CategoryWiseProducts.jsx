...
const CategoryWiseProducts = () => {
  ...
  const [filters, setFilters] = useState({
    ...
    size: "",
    brand: "",
    subcatid: "", // Make sure this matches the subcategory field in your data
    color: ""
  });
  const [filteredProducts, setFilteredProducts] = useState([]);
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // Update the price range filter with the correct max price
        setFilters(prevFilters => ({
          ...prevFilters,
          priceRange: [0, maxSellPrice]
        }));
  ...
  useEffect(() => {
    ...
    // Price range filter
    filtered = filtered.filter(product =>
      product.sellprice >= filters.priceRange[0] && product.sellprice <= filters.priceRange[1]
    );
    ...
    if (filters.size) {
      filtered = filtered.filter(product =>
        ...
        product.sizes.some(sizeObj =>
          typeof sizeObj.size === 'string' && sizeObj.size.toUpperCase() === filters.size.toUpperCase()
        )
      );
    }
    ...
    if (filters.brand) {
      filtered = filtered.filter(product =>
        product.brandname && product.brandname.toLowerCase().includes(filters.brand.toLowerCase())
      );
    }
    ...
    if (filters.subcatid) {
      filtered = filtered.filter(product =>
        // Ensure you are checking the correct subcategory field here (adjust if necessary)
        product.subcategoryId && product.subcategoryId === parseInt(filters.subcatid, 10)
      );
    }
    ...
    if (filters.color) {
      filtered = filtered.filter(product =>
        ...
        product.sizes.some(sizeObj =>
          sizeObj.colors &&
          Array.isArray(sizeObj.colors) &&
          sizeObj.colors.some(colorObj =>
            colorObj.color.toUpperCase() === filters.color.toUpperCase()
          )
        )
      );
    }
