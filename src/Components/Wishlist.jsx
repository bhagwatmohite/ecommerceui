...
//   return (
//     <Container className="wishlist-container" fluid >
//       <Row>
//         {/* <Col md={3}>
...
//                 <Row>
//                   <Col md={2}>
//                     <Image src={`http://localhost:8080/ColorImages/${item.firstImage}`} alt={item.name} className="img-fluid" />
//                   </Col>
//                   <Col md={8}>
      ...
      try {
        ...
        if (response.ok) {
          ...
          const promises = productIds.map(async (productId) => {
            ...
            // Extract firstImage from the product data
            const firstImage = productData.sizes?.[0]?.colors?.[0]?.images?.[0] || '';
            return { ...productData, firstImage }; // Add firstImage to the product object
          });
            ...
            <Card key={item.id} className="mb-3">
              <Card.Body>
                <Row>
                  <Col md={2}>
                    <Image
                      src={`http://localhost:8080/ColorImages/${item.firstImage}`}
                      alt={item.name}
                      className="img-fluid"
                    />
                  </Col>
                  <Col md={8}>
                    <Card.Title>{item.name}</Card.Title>
                    <Card.Text className="text-muted">Brand: {item.brandname}</Card.Text>
                    <Card.Text>
                      <span className="fw-bold">₹{calculateNewSellPrice(item.sellprice)}</span>
                      <span className="text-muted ms-2 text-decoration-line-through">₹{item.price}</span>
                      <span className="text-success ms-2">
                        {calculateDiscount(item.price, calculateNewSellPrice(item.sellprice))}% Off
                      </span>
                    </Card.Text>
                    <div className="d-flex align-items-center">
                      <Button variant="outline-secondary" size="sm" onClick={() => handleDecreaseQuantity(item.id)}>-</Button>
